...
const ModalUnits = () => {
  ...
  const isRegistrationModalShown = useSelector(
    (state) => state.registrationSlice.isRegistrationModalShown
  );
  const isPreloaderSliceShown = useSelector(
    (state) => state.preloader.isPreloaderSliceShown
  );

  const handleUnitsRegistrationModal = () => {
    dispatch(hideUnitsRegistrationSlice());
  };

  return (
    <div className={`modal ${isRegistrationModalShown ? "active" : ""}`}>
      <div className="modal-wrapper">
        ...
        <div className="modal-inner mt-2">
          ...
          <div className="flex">
            <button
              className="text-sm py-[0.3rem] px-[2rem] bg-green-700 rounded-sm text-white mt-[1rem]"
              onClick={handleUnitsRegistrationModal}
            >
              Done
            </button>
            <button
              className="text-sm py-[0.3rem] px-[2rem] bg-red-700 rounded-sm text-white mt-[1rem] ml-[1rem]"
              onClick={handleUnitsRegistrationModal}
            >
              Cancel
            </button>
